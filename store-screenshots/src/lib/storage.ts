...
function isSlideLike(value: unknown): value is Slide {
  return !!value && typeof value === "object" && typeof (value as Slide).id === "string";
}
...
function mergeWithDefaults(parsed: Partial<ProjectState>): ProjectState {
    ...
    ? Object.fromEntries(
        Object.entries(parsed.slidesByDevice).map(([device, slides]) => [
          ...
          Array.isArray(slides)
            ? slides.filter(isSlideLike).map((slide) => migrateSlide(slide))
            : [],
        ]),
      )
    : {};
  const locales = Array.isArray(parsed.locales)
    ? parsed.locales.filter((locale): locale is string => typeof locale === "string" && !!locale.trim())
    : undefined;
  ...
  if (typeof merged.locale !== "string" || !merged.locales.includes(merged.locale)) {
...
export function useProject() {
  ...
  const fileSaveChain = useRef<Promise<void>>(Promise.resolve());
  ...
  // Debounced autosave to localStorage always; file only when the project API is ready.
  useEffect(() => {
    if (!hydrated) return;
    ...
    timer.current = setTimeout(() => {
      ...
      if (!fileReady) {
        setSavedAt(localResult.ok ? Date.now() : null);
        setSaveError(localResult.ok ? null : localResult.error);
        return;
      }
      // Serialize file writes so a slower older save cannot overwrite a newer one.
      const snapshot = state;
      fileSaveChain.current = fileSaveChain.current
        .catch(() => undefined)
        .then(async () => {
          const fileResult = await saveToFile(snapshot);
