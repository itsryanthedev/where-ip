...
function EditableText({
  ...
  const handlePaste = (e: React.ClipboardEvent<HTMLDivElement>) => {
    ...
    e.preventDefault();
    const pasted = e.clipboardData.getData("text/plain").replace(/\u00a0/g, " ");
...
function Movable({
  ...
  const [liveRotation, setLiveRotation] = React.useState(rotation);
  React.useEffect(() => {
  ...
  const rotateCleanupRef = React.useRef<(() => void) | null>(null);
  ...
  function startRotate(e: React.PointerEvent<HTMLButtonElement>) {
    ...
    const stopRotate = () => {
      ...
      if (rotateCleanupRef.current === stopRotate) rotateCleanupRef.current = null;
      onChange({
        x: base.x,
        y: base.y,
        width: base.width,
    ...
    rotateCleanupRef.current?.();
    rotateCleanupRef.current = stopRotate;
  ...
  return (
    <Rnd
      ...
      onDragStop={(_e, d) => {
        ...
        onChange({ ...next, rotation: liveRotation, zIndex });
      }}
