...
export function ScreenshotEditor() {
  ...
  const exportingLockRef = React.useRef(false);
  ...
  const duplicateSlide = React.useCallback(
    (id: string) => {
      ...
      setState((prev) => {
        const current = prev.slidesByDevice[prev.device] || [];
        const currentIdx = current.findIndex((s) => s.id === id);
        if (currentIdx === -1) return prev;
        const next = [...current.slice(0, currentIdx + 1), copy, ...current.slice(currentIdx + 1)];
        return {
          ...prev,
          slidesByDevice: { ...prev.slidesByDevice, [prev.device]: next },
        };
  ...
  async function exportAll() {
    if (exportingLockRef.current || exporting) return;
  ...
  return (
    <div className="flex h-screen flex-col overflow-hidden bg-background">
      ...
      {/* Off-screen export container — mount only while exporting. */}
      {busy && currentSlides.length > 0 && (
      <div
        aria-hidden
        style={{
          position: "absolute",
