...
**Maintainer-local only.** Run with `pnpm dev` / `bun dev` on localhost. Do not deploy or expose `next start` on a network — `/api/project` and `/api/upload` write to the local filesystem and are restricted to localhost requests.

## Quick start

```bash
pnpm install   # or bun / yarn / npm
